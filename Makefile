...
BK_DEPS = /home/nox/src/mysql++svn/mysqlpp/bk-deps
srcdir = .
...
COND_USE_SOTWOSYMLINKS_1___mysqlpp___so_symlinks_cmd = (cd .; rm -f \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX) \
	...
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX))
__mysqlpp___so_symlinks_cmd = $(COND_USE_SOTWOSYMLINKS_1___mysqlpp___so_symlinks_cmd)
COND_USE_SOVERSOLARIS_1___mysqlpp___so_symlinks_cmd = (cd .; rm -f \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX); $(LN_S) \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf3) \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX))
#__mysqlpp___so_symlinks_cmd = $(COND_USE_SOVERSOLARIS_1___mysqlpp___so_symlinks_cmd)
COND_USE_SOTWOSYMLINKS_1___mysqlpp___so_symlinks_inst_cmd = rm -f \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX) \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf2); $(LN_S) \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf3) \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf2); $(LN_S) \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf2) \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX)
__mysqlpp___so_symlinks_inst_cmd = $(COND_USE_SOTWOSYMLINKS_1___mysqlpp___so_symlinks_inst_cmd)
COND_USE_SOVERSOLARIS_1___mysqlpp___so_symlinks_inst_cmd = rm -f \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX); $(LN_S) \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf3) \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX);
#__mysqlpp___so_symlinks_inst_cmd = $(COND_USE_SOVERSOLARIS_1___mysqlpp___so_symlinks_inst_cmd)
COND_USE_SOTWOSYMLINKS_1___mysqlpp___so_symlinks_uninst_cmd = rm -f \
	$(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf2) \
	$(LIBPREFIX)mysqlpp.$(DLLIMP_SUFFIX)
__mysqlpp___so_symlinks_uninst_cmd = $(COND_USE_SOTWOSYMLINKS_1___mysqlpp___so_symlinks_uninst_cmd)
#__mysqlpp___so_symlinks_uninst_cmd \
...
$(LIBPREFIX)mysqlpp_ssqls2parse$(LIBEXT): $(SSQLS2PARSE_OBJECTS) $(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf3) $(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf3)
	rm -f $@
...
test_ssqls2$(EXEEXT): $(TEST_SSQLS2_OBJECTS) $(DLLPREFIX)mysqlpp$(__mysqlpp___targetsuf3) $(LIBPREFIX)mysqlpp_ssqls2parse$(LIBEXT)
	$(CXX) -o $@ $(TEST_SSQLS2_OBJECTS)     -lmysqlpp_ssqls2parse -WI,--as-needed -L. -L/usr/local/lib/mysql -lmysqlclient   $(LDFLAGS)  -lmysqlpp -lmysqlpp_ssqls2parse $(LIBS)
...
mysql++-3.1.1: 
	...
	mysql++.* osver Wishlist mysql++-3.1.1
	for d in vc200? ; do \
	mkdir -p mysql++-3.1.1/$$d ; \
	cp $$d/*.sln $$d/*.vcproj mysql++-3.1.1/$$d ; \
	done
	cp config/*.m4 mysql++-3.1.1/config
	cp -RL doc/README* doc/pdf doc/ssqls-pretty mysql++-3.1.1/doc
	for f in LICENSE.txt Makefile mktxt README.txt \*.dbx \*.in \*.xsl ; \
	do \
	cp doc/userman/$$f mysql++-3.1.1/doc/userman ; \
	done
	for d in refman userman ; do \
	for f in *.css *.html ; do \
	cp doc/html/$$d/$$f mysql++-3.1.1/doc/html/$$d ; \
	done ; \
	done
	cp doc/html/refman/*.png mysql++-3.1.1/doc/html/refman
	cp examples/*.cpp examples/*.h examples/*.jpg mysql++-3.1.1/examples
	for d in mfc wforms ; \
	do \
	mkdir -p mysql++-3.1.1/examples/vstudio/$$d ; \
	cp examples/vstudio/$$d/* mysql++-3.1.1/examples/vstudio/$$d ; \
	done
	cp lib/*.cpp lib/*.h lib/*.in lib/*.pl mysql++-3.1.1/lib
	cp test/*.cpp mysql++-3.1.1/test
	cp ssx/*.cpp ssx/*.h ssx/*.in mysql++-3.1.1/ssx
	cd mysql++-3.1.1 ; ./bootstrap nodoc nomaint
	rm -f mysql++-3.1.1/Makefile
	rm -f mysql++-3.1.1/config.log
	rm -f mysql++-3.1.1/config.status
	rm -fr mysql++-3.1.1/autom4te.cache

abicheck: 
	sed -e s/`sh version`/3.0.9/ < abi.xml > ../3.0.9/abi.xml
	~/src/abi-compliance-checker/head/abi-compliance-checker.pl \
	-lib mysqlpp -d1 ../3.0.9/abi.xml -d2 abi.xml

dist: doc mysql++-3.1.1
	./checkvc8
	tar czf mysql++-3.1.1.tar.gz mysql++-3.1.1
	rm -rf mysql++-3.1.1

rpm: doc dist
	cp mysql++-3.1.1.tar.gz /usr/src/redhat/SOURCES
	cd /usr/src/redhat/SPECS
	rpmbuild -ba mysql++.spec

ebuild: dist
	cp mysql++-3.1.1.tar.gz /usr/portage/distfiles
	cp mysql++.ebuild /usr/portage/dev-db/mysql++/mysql++-3.1.1.ebuild
	ebuild /usr/portage/dev-db/mysql++/mysql++-3.1.1.ebuild digest
